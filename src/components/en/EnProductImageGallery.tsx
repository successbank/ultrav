...
// 영문 사이트용 상품 이미지 갤러리 — ProductImageGallery와 동일 구조, 텍스트만 영문
interface EnProductImageGalleryProps {
  productName: string;
...
export default function EnProductImageGallery({
  productName,
  mainImage,
  additionalImages,
  discount = 0,
}: EnProductImageGalleryProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [currentIndex, setCurrentIndex] = useState(0);

  const allImages = [...(mainImage ? [mainImage] : []), ...additionalImages];
