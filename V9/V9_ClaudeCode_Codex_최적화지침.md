...
### 4.1 권장 Hook 목록 (V9 기존 + 신규)
| Hook | 시점 | 동작 |
|---|---|---|
| `pre-development-search.sh` | 작업 시작 직전 | 공유DB 유사 사례 검색 → 컨텍스트 주입 |
| `pre-edit-pii-scan.sh` | 파일 편집 직전 | 시크릿·PII 정규식 차단 |
| `post-edit-format.sh` | 편집 직후 | prettier/eslint/swiftformat 자동 |
| `pre-commit-typecheck.sh` | 커밋 전 | tsc + 영향받은 패키지 typecheck |
| `pre-pr-eval-gate.sh` | PR 생성 직전 | 골든 케이스 회귀 비교 |
| `post-error-to-db.sh` | 에러 발생 시 | 공유DB `errors` 적재 |
| `post-task-to-db.sh` | 작업 완료 시 | 공유DB `tasks` 결과 기록 |
| `pre-cross-platform-build.sh` | RN/Tauri 빌드 전 | 영향 plat 매트릭스 산정 |

### 4.2 작성 원칙
- exit code 0/1로 결정적 차단
- 표준 출력에 "왜 막혔는지" 한 줄만 (Claude가 자동 해소)
...
### 7.3 등록
- Claude Code: `.claude/settings.json` 의 `mcpServers`
